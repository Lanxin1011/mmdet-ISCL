...
class StandardRoIHead2DeCon(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def calc_con_loss(self, pos_roi_feats, pos_roi_labels):
        ...
        con_loss = 0
        # max_pos_num = 100
        # select_idx = random.sample(range(0, len(pos_roi_feats)), min(max_pos_num, len(pos_roi_feats)))
        # # print(select_idx)
        # # 对正roi进行随机采样后进行对比损失的计算，避免后期因正roi过多导致训练进程严重变慢
        # select_pos_roi_feats = []
        # select_pos_roi_labels = []
        # pos_roi_feats = avg_pool(pos_roi_feats).flatten(xxx)
        # pos_roi_feats = pos_roi_feats / pos_roi_feats.sum(dim)
        for i in range(len(pos_roi_feats)):  ## select_idx
            ...
            # select_pos_roi_feats.append(pos_roi_feats[i])
            # select_pos_roi_labels.append(pos_roi_labels[i])

        # assert len(select_pos_roi_labels) == len(select_pos_roi_feats) == len(select_idx)
