...
class StandardRoIHead2DeCon(BaseRoIHead, BBoxTestMixin, MaskTestMixin, nn.Module):
    ...
    def __init__(self,
        ...
        #     if self.con_loss_type == 'triplet':
        #         self.triplet_loss = nn.TripletMarginLoss(margin=0.5, p=2)  # MOVE TO INIT

        super(StandardRoIHead2DeCon, self).__init__(
        ...
        if self.enable_con_loss:
            ...
            if self.con_loss_type == 'triplet':
                self.triplet_loss = nn.TripletMarginLoss(margin=0.5, p=2)  # MOVE TO INIT
                print(f'triplet_loss:{self.triplet_loss}')
    ...
    def _bbox_forward_train(self, stage, x, sampling_results, gt_bboxes,
        ...
        bbox_targets = self.bbox_head[stage].get_targets(
            sampling_results, gt_bboxes, gt_labels, rcnn_train_cfg)   ###### 加上[:-1]
        ### 如果是stage2， 则多传入一个参数bbox_targets[0]用来筛选正样本roi
        bbox_results = self._bbox_forward(stage, x, rois)

        loss_bbox = self.bbox_head[stage].loss(bbox_results['cls_score'],     ###### 这里报错了！ 因为输入的
    ...
    def forward_train(self,
        ...
        for i in range(self.num_stages):
            ...
            sampling_results = []
            '''
            sampling_results中的每一个元素都是资格字典，字典中包含以下key-value pair： 'neg_bboxes' 'neg_inds' 
                                                                             'num_gts' 'pos_assigned_gt_inds' 
                                                                            'pos_bboxes' 'pos_inds' 'pos_is_gt'
            ...
            # refine bboxes   standard_roi_head中没有的，用来修正矩形框
            if i < self.num_stages - 1:
                pos_is_gts = [res.pos_is_gt for res in sampling_results] ###### 存放batchsize(4)张图片中正样本roi是gt的索引，
                                                                         # 注意：在一张图片中正样本的索引在所有roi（512个）索引列表中的最前面
                # print(pos_is_gts)
                # bbox_targets is a tuple
                roi_labels = bbox_results['bbox_targets'][0]  ### 'bbox_targets'存放的tuple内容详见 bbox_head:get_targets()
                # print(roi_labels)
                # print(roi_labels.shape)

                # roi_gt_bboxes = bbox_results['bbox_targets'][4]  # 存放batchsize张图片中所有roi（当batchsize=4时，roi_gt_bboxes.shape=torch.Size([2048, 4])）
                # print(roi_gt_bboxes[0]) ###### roi_gt_bboxes 和 rois中存放内容的差异仅在于rois多了一列用来存放batch_num(0~batchsize)
                # rois = bbox2roi([res.bboxes for res in sampling_results])  ###### roi.shape=torch.Size([2048, 5]) 其中[:,0]存放的是batch_num
                # print(rois[0])
                ...
                with torch.no_grad():
                    ...
                    # roi_labels = torch.where(         # 若满足条件，则从cls_score[:, :-1].argmax(1)中取元素，否则，从roi_labels中取元素
            ...
            if self.enable_con_loss:
                if i == self.num_stages - 1:
                    ...
                    assert roi_feats.shape[0]==roi_labels.shape[0], 'The batchsize of roi features and roi lables should be the same!'
                    # get positive roi index
                    roi_labels_cpu = np.array(roi_labels.cpu())
                    pos_indx = np.argwhere(roi_labels_cpu < 10)
                    ...
                    ###### randomly select no larger than 200 positive roi features
                    max_pos_num = 200   ###### 为什么会完全没有收敛???
                    select_idx = random.sample(pos_indx, min(max_pos_num, len(pos_indx)))
                    pos_roi_feats = roi_feats[select_idx]    ### 此时得到的不是列表而是一个（k, 256, 14, 14）的张量
                    pos_roi_labels = roi_labels[select_idx]

                    assert len(select_idx) == len(pos_roi_feats) == len(pos_roi_labels)

                    con_loss = self.calc_con_loss(pos_roi_feats, pos_roi_labels)
                    if con_loss != 0:  # and con_loss is not
                        losses[f's{i}.loss_con'] = self.con_w * con_loss
                    else:
                        losses[f's{i}.loss_con'] = torch.tensor(0.).to(self.device)
                    print(losses[f's{i}.loss_con'])
    ...
    def calc_con_loss(self, pos_roi_feats, pos_roi_labels):
        ...
        con_loss = 0  # 这里改成张量0
        valid_cnt = 0  # 改成从1开始
        ### 对正roi进行随机采样后进行对比损失的计算，避免后期因正roi过多导致训练进程严重变慢

        pos_roi_feats = avg_pool(pos_roi_feats).flatten(2)  # (k, 256, 1)
        pos_roi_feats = torch.true_divide(pos_roi_feats, torch.norm(pos_roi_feats, dim=1).reshape(len(pos_roi_feats),-1,1))  # (k, 256, 1)
                                        # 将得到的pos_roi_feats再经过torch.norm(pos_roi_feats, dim=1)得到的应该是全为1的向量
        labels = [[] for i in range(10)]  # 存放具有相同类别的元素下标
        # assert False
        for i in range(len(pos_roi_labels)):
            labels[pos_roi_labels[i]].append(i)
        # assert False
        pos_idxs = []
        neg_idxs = []
        for i in range(len(pos_roi_feats)):
            label = copy.deepcopy(labels)
            pos_idx = copy.deepcopy(label[pos_roi_labels[i]])
            pos_idx.remove(i)
            neg_idx = label  # [i for k in range(10) for i in labels[k] if k != pos_roi_labels[i]]
            neg_idx.remove(label[pos_roi_labels[i]])
            neg_idx = [j for k in neg_idx for j in k]
        ...
        for j in range(len(pos_roi_feats)):
            ...
            #     if len(pos_idxs[j])<1 or len(neg_idxs[j])<3:
            #         print("the feature number is less than 1 or 3!")
            #         continue
            con_loss_single = self._calc_conloss_single(j, pos_roi_feats, pos_roi_labels, pos_idxs[j], neg_idxs[j])
            # print(con_loss_single)
            if con_loss_single != 0:
                valid_cnt += 1
    ...
    def _calc_conloss_single(self, q_indx, roi_feats, roi_labels, pos_idxs, neg_idxs, max_num=30, tao=0.1):
        '''
        ...
        q_feat = roi_feats[q_indx].permute(1,0) # torch.size([256, 1]) --> torch.size([1, 256])

        if len(pos_idxs) == 0 or len(neg_idxs) == 0:  # 这里保证了不会有没有正样本或是负样本的query输入到后续函数中
            return 0

        else:
            pos_feats = roi_feats[pos_idxs].permute(0, 2, 1)   # torch.size([k, 1, 256])
            neg_feats = roi_feats[neg_idxs].permute(0, 2, 1)
            if self.con_loss_type == 'infoNCE':
        ...
        if select == 'random':
            ...
            for j in range(len(neg_feats)):
                if j > max_num:
                    ...
                    cur_logit = torch.exp(torch.true_divide(torch.mm(q_feat.view(1, -1), neg_feats[j].view(-1, 1)), tao))
            ...
            if len(pos_feats) < 1 or len(neg_feats) < 3:
                ...
                if aspect == True:  # 说明负例个数除以3小于正例， 以负例数进行选择
                    n_neg = 3*(len(neg_feats)//3)  # 用更精简的语言代替 if-else
                ...
                # select neg and pos features according to i_neg and i_pos
                neg_feats = neg_feats[i_neg]
                pos_feats = pos_feats[i_pos]

                neg_logits = torch.exp(torch.true_divide(torch.mm(q_feat.view(-1, 256), neg_feats.squeeze().view(256, -1)), tao)).sum()
                pos_logits = torch.exp(torch.true_divide(torch.mm(q_feat.view(-1, 256), pos_feats.squeeze().view(256, -1)), tao)).sum()
    ...
    def triplet_single(self, q_feat, pos_feats, neg_feats):
        # 为了利用pytorch自带的nn.TripletMarginLoss() ，故这里会将q_feat, pos_feats, neg_feats都扩展成相同的(N, D)
        max_len = max(len(pos_feats), len(neg_feats))
        q_feat = torch.repeat_interleave(q_feat, max_len, 0)  # torch.size([max_len, 256])
        logit = int(len(pos_feats)-len(neg_feats))
        pos_feats = pos_feats.squeeze(1)  # torch.size([k1, 256])
        neg_feats = neg_feats.squeeze(1)  # torch.size([k2, 256])
        # print(pos_feats.shape)
        if logit == 0:
            pass
        elif logit > 0:
            tmp_list = neg_feats[0].repeat(abs(logit), 1)
            neg_feats = torch.cat((neg_feats, tmp_list), 0)
        else:
            tmp_list = pos_feats[0].repeat(abs(logit), 1)  # [pos_feats[0] for i in range(abs(logit))]
            pos_feats = torch.cat((pos_feats, tmp_list), 0)

        assert q_feat.shape == pos_feats.shape == neg_feats.shape
        # triplet_loss = nn.TripletMarginLoss(margin=0.5, p=2)  # MOVE TO INIT
        output = self.triplet_loss(q_feat, pos_feats, neg_feats)
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        ...
        for i in range(self.num_stages):
            ...
            if i==self.num_stages-1:
                cls_score = cls_score.split(num_proposals_per_img, 0)     ###### 这里报错了  ###### 只在第二个（最后一个stage）进行这个操作
            else:
                pass
            ...
            ms_scores.append(cls_score)  ###### 那么对于不是最后一个stage的情况来说，这里append进去的就是None
            ...
            if i < self.num_stages - 1:
                ...
                for j in range(num_imgs):
                    if rois[j].shape[0] > 0:
                        # bbox_label = cls_score[j][:, :-1].argmax(dim=1)             ###### 这里改为None
                        bbox_label = None
