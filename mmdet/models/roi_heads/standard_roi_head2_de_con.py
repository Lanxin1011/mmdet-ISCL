...
class StandardRoIHead2DeCon(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
                 ...
                 num_stages=2,
                 enable_con_loss=False,  # whether to include the CLM module (i.e. the InfoNCE+ loss)
                 con_loss_type=None,  # choose different types of contrastive loss, currently only InfoNCE+ loss and Triplet loss (in StandardRoIHead2DeTri) are supported
                 con_weight=None,  # loss weight
                 tau=0.1,  # tau refers to the temperature coefficient in the InfoNCE+ loss
                 ratio=3,  # ratio indicates the value of neg_sample:pos_sample
        ...
        if self.enable_con_loss:
            ...
            if self.con_loss_type == 'infoNCE':
                self.tau = tau
                self.ratio = ratio
                print(f'###### tau: {self.tau}, ratio: {self.ratio} ######')
    ...
    def forward_train(self,
        ...
        for i in range(self.num_stages):
            ...
            sampling_results = []
            '''
            Each element in the 'sampling_results' is a dictionary, and each dictionary includes the following key-value pairs:
            ...
            if i < self.num_stages - 1:
                ...
                # note that the index of positive samples are in the front part of the index list.
                # print(pos_is_gts)
    ...
    def _calc_conloss_single(self, q_indx, roi_feats, roi_labels, max_num=30):
        '''
        ...
        if len(pos_feats) == 0 or len(neg_feats) == 0:
            ...
            if self.con_loss_type == 'infoNCE':
                con_loss_single = self.info_NCE_single(q_feat, pos_feats, neg_feats, max_num)
    ...
    def info_NCE_single(self, q_feat, pos_feats, neg_feats, max_num, select='rule'):
        pos_logits = 0
        neg_logits = 0
        flag = 1
        if select == 'random':
            k = random.randint(0, len(pos_feats) - 1)
            pos_logits = torch.exp(torch.true_divide(torch.mm(q_feat.view(1, -1), pos_feats[k].view(-1, 1)), self.tau))
            ...
            for j in range(len(neg_feats)):
                if j > max_num:
                    ...
                    cur_logit = torch.exp(
                        torch.true_divide(torch.mm(q_feat.view(1, -1), neg_feats[j].view(-1, 1)), self.tau))
        ...
        elif select == 'rule':  ### all the 3 --> self.ratio
            if len(pos_feats) < 1 or len(neg_feats) < self.ratio:
                flag = 0
            else:
                aspect = len(neg_feats) // self.ratio < len(pos_feats)
                if aspect == True:  # means num_neg/3 < num_pos --> select according to neg samples
                    n_neg = self.ratio * (len(neg_feats) // self.ratio)
                    n_pos = len(neg_feats) // self.ratio
                    if n_neg > max_num:
                        n_neg = max_num
                        n_pos = max_num // self.ratio
                else:
                    n_neg = self.ratio * len(pos_feats)
                    n_pos = len(pos_feats)
                    if n_pos > max_num // self.ratio:
                        n_pos = max_num // self.ratio
                        n_neg = max_num
                ...
                for i in i_neg:
                    cur_logit = torch.exp(
                        torch.true_divide(torch.mm(q_feat.view(1, -1), neg_feats[i].view(-1, 1)), self.tau))
                    neg_logits += cur_logit
                for j in i_pos:
                    cur_logit = torch.exp(
                        torch.true_divide(torch.mm(q_feat.view(1, -1), pos_feats[j].view(-1, 1)), self.tau))
