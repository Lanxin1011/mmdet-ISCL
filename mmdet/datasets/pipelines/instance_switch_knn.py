...
class InstanceSwitchKNN:
    ...
    def __init__(self, data_root="data/GF/airplane/", switch_prob=0.5,
        ...
        # self.file_name_list = set()
        # for image in images:
        #     self.file_name_list.add(image['file_name'])
        # self.switch_name_list = set()

        img_to_instance = {}
        img_to_instance_wo_name = []

        # self.switch_vis_path = '/home/zlx/iscl_mmdetection/work_dirs/rebuttal/online_IS2_faster_r50_on_newset/switch_vis_rectangle/'  # switch_vis_rectangle
        # try:
        ...
        for i in tqdm.trange(len(crop_list)):
            ...
            if obj_crop.shape[0] * obj_crop.shape[1] < 1500:  # <1500
                continue
    ...
    def __call__(self, results):
        ...
        for ii, bridge_ in enumerate(bridges):
            ...
            switch_warped = cv2.warpPerspective(switch_target, M, (img_cv.shape[1], img_cv.shape[0]))
            img_cv[canvas == 255] = switch_warped[canvas == 255]
            # print(f'{ori_cat_ids[ii]} --> {target_id}')
            ori_cat_ids[ii] = target_id
        # visualization
        if num_switch >= 1:
            cv2.imwrite(self.switch_vis_path + filename, img_cv)


            # print(ori_cat_ids[ii])
        # if self.switch_name_list == self.file_name_list:
        #     print("YES!")
        # else:
        #     cv2.imwrite(self.switch_vis_path + filename, img_cv)
        #     self.switch_name_list.add(filename)
