...
class PlaneDataset(CustomDataset):
    ...
    def modify_switch_cls(self, cm):
        ####### how to choose the classes to be modified
        l = len(cm)

        ##### method1
        # select two categories which are most likely to be confused with each other
        # max_ratio = 0.0
        # for i in range(l-1): ### 不考虑other类？
        #     ratio = 0.0
        #     tp = cm[i, i]
        #     fp_max = 0
        #     fp_max_id = -1 
        #     for j in range(l-1):
        #         if cm[i, j] > fp_max and i != j:
        #             fp_max, fp_max_id = cm[i, j], j
        #             ratio = 1.0 * fp_max/tp
        #     if ratio > max_ratio:
        #         max_ratio = ratio
        #         ii, jj = i, j
        # print(ii, jj)
        ##### method2
        # select two categories which are most likely to be confused with other classes
        # max_ratio = []
        # for i in range(l):
        #     ratio = 0.0
        #     tp = cm[i, i]
        ...
        # sorted_ratio = [index for index, value in sorted(enumerate(max_ratio), key=itemgetter(1), reverse=True)]  # 降序
        # ii, jj = sorted_ratio[0], sorted_ratio[1]
        # print(ii, jj)
        # return [ii, jj]

        ##### method3
        # select K categories which has classification precision less than the average classification precision
        tp_ratio = []
        for i in range(l):
            # print(cm[i,:])
            tpfp = sum(cm[i,:])
            ...
            if tpfp == 0:
                ...
                r = tp/tpfp
            tp_ratio.append(r)
        tp_avg = sum(tp_ratio)/l
        # print(tp_avg)
        switch_cls = [i for i, tp_r in enumerate(tp_ratio) if tp_r < tp_avg]
        # print(tp_ratio)
        print(switch_cls)
    ...
    def evaluate(self,
        ...
        gts_path = '/mapai/haowenguo/code/SPL/mmdetection/plane_cm/trainval_labels_gf/'
        # switch_cls_path = '/jizhi/jizhi2/worker/trainer/switch_cls.txt'
        # save_txt_path = '/jizhi/jizhi2/worker/trainer/det_results_0.1.txt'
        switch_cls_path = '/mapai/haowenguo/code/SPL/mmdetection/switch_cls.txt'
        save_txt_path = '/mapai/haowenguo/code/SPL/mmdetection/det_results_0.1.txt'
        map_thr = 0.1
        ...
        for metric in metrics:
            ...
            if metric == 'proposal':
                ...
                if flag and (ap[1] > map_thr):
                    switch_cls = self.modify_switch_cls(cm)
                    os.remove(switch_cls_path)
                    with open(switch_cls_path, 'w') as f:
                        cls_txt = ' '.join(str(i) for i in switch_cls)
                        f.write(cls_txt)
                    print(f'The new switch_cls is: {cls_txt}')
                # if num_val == len(results):
